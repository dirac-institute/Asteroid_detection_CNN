"""Two-stage postprocessing v2 for the diffim NN.

Stage 1 = NN candidate extraction (`extract_candidates`).
Stage 2 = a RandomForest reranker over a feature set spanning:
  * candidate geometry / probability statistics (from extract_candidates)
  * symmetric matched filter (PCA-derived line) + fixed long-line variants
  * 64x64 local diffim morphology around the centroid
  * multi-angle matched filter (12 angles, lengths 30/50/80)
  * low-threshold PCA matched filter (window 32/48/64, t 0.05/0.10/0.20)
  * orient-aware features built from the model's orient_sin / orient_cos /
    line-aggregator outputs

The trained RF is shipped at:
  experiments/diffim_runs/pilot_v7/ckpts/rf_postproc_v2.pkl
...
Use this module from notebook / scripts as:
    ...
    cand_df = compute_v2_features(panel_probs, diffims, sin, cos, agg,
                                   real_labels=stack_fp)
    cand_df = apply_rf_v2(cand_df, rf=rf_model)
    masks = materialize_label_mask_v2(cand_df[cand_df.score_rf >= 0.04],
                                       panel_probs_dict, panel_probs.shape)
"""
...
# ---------------------------------------------------------------------------
# Feature set (must stay in lock-step with rf_postproc_v2.pkl)
# ---------------------------------------------------------------------------

# (win, low_thr, line_length) tuples used in iter11 — keep in this order.
