...
def main():
    ...
    ap.add_argument("--region-from", default=str(REPO / "ADCNN/pipelines/heliolinc/run_disco/adcnn_dets_full.csv"),
                    help="csv whose ra/dec bounding box defines the discovery region (for apples-to-apples)")
    ap.add_argument("--out", default=str(REPO / "ADCNN/pipelines/heliolinc/run_disco/diasources.csv"))
    a = ap.parse_args()
