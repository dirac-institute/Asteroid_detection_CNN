...
(reg2 segmentation model + reg2 neg5 RF), produce a detection catalog, link with HelioLinC, and
...
Cross-env reality: Butler lives in lsst_distrib; the scripted segmentation model needs torch
...
(node-local lscratch or shared mem) — never the full dataset on disk:
  ...
  CONSUMER (asteroid_cnn, 4 GPU workers): reg2 segmentation model -> compute_v2_features -> reg2 RF
...
- Bottlenecks: (1) diffim fetch ~19s I/O (parallelize across ~60 CPU workers ->
  ...
  with many candidates). The 4 GPUs handle segmentation model inference cheaply (~0.5s/panel).
...
- reg2 segmentation model (`pilot_seg_reg2/ckpts/segmentation_reg2_best_scripted.pt`) + reg2 RF
...
2. `pipeline_consumer.py` (asteroid_cnn, 4 GPU): consume buffer -> reg2 segmentation model+RF candidates
