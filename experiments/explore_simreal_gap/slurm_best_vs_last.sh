...
python3 -u -m ADCNN.inference.diffim_export --ckpt "$CK/best.pt" --out "$CK/v7_big_best_scripted.pt" --no-optimize
python3 -u -m ADCNN.inference.diffim_export --ckpt "$CK/last.pt" --out "$CK/v7_big_last_scripted.pt" --no-optimize
