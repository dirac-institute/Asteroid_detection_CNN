...
#SBATCH --output=/sdf/home/m/mrakovci/logs/ADCNN_gen_big2_%j.out
set -eo pipefail
...
OUT="$REPO/DATA_DIFFIM_realistic_big2"; mkdir -p "$OUT"
echo "=== gen big2 (seed 789, ~2000 new validated panels, all existing+test excluded) === $(date -Is)"
srun python3 -u simulate_inject_diffim.py \
  ...
  --train-test-split 0.98 --random-subset 2000 \
  ...
  --chunks 128 --seed 789 --realistic-trail \
  --exclude-pairs-csv "$REPO/DATA_DIFFIM/test_5sigma/test.csv" "$REPO/DATA_DIFFIM/test_real/test.csv" "$REPO/DATA_DIFFIM_realistic/train.csv" "$REPO/DATA_DIFFIM_realistic_big/train.csv" \
  --where "instrument='LSSTCam' AND day_obs>=20250801 AND day_obs<=20250921 AND band in ('u','g','r','i','z','y')"
echo "GEN BIG2 DONE $(date -Is)"
